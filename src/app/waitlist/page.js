...
const payload = {
    name: storyData.womanName,
    story: storyData.story,
    impact: storyData.impact,
};

try {
    const response = await fetch('https://viragovox.marvelly.com.ng/story', {
        method: 'POST',
        headers: {
            'Content-Type': 'application/json',
            'Accept': 'application/json',
    ...
    if (!response.ok) {
        ...
        Swal.fire({
            title: 'Success!',
            text: result.message || 'Thank you for sharing her story!',
            icon: 'success',
        });
        setStoryData({
            womanName: '',
